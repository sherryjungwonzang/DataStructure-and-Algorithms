...
var bestTimeStock2 = (prices) => {
  ...
  //looping through - starting from index 1 to compare with index 0 value
  for (let i = 1; i < prices.length; i++) {
    //if the previous value is less than current value - meaning upwards trajectory
    if (prices[i] > prices[i - 1]) {
      //calculating the difference between current and previous value
      let diff = prices[i] - prices[i - 1];

      //updating total
      total += diff;
    }
    //if the current value is less than previous value - meaning no peak
    //increment i
  }
  return total;
}
