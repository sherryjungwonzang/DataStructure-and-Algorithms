...
var longestCommonSubsequence = (text1, text2) => {
  //create variables of length of text1 and text2
  let m = text1.length;
  let n = text2.length;

  //creating DP matrix
  let dp = Array.from(Array(m + 1), () => new Array(n + 1).fill(0)); //just fill the entire matrix as 0

  //loop through text1 and text2
  //setting to start i = 1 because we initially set the first row to 0
  for (let i = 1; i <= m; i++) {
    for (let j = 1; j <= n; j++) {
      if (text1[i - 1] === text2[j - 1]) {
        //when we find the match
        dp[i][j] = dp[i - 1][j - 1] + 1; //we can populate the DP array at i and j to the diagonal
      } else { //if there is no match
        dp[i][j] = Math.max(dp[i - 1][j], dp[i][j - 1]); //max between the left value and the value above
      }
    }
  }
  return dp[m][n]; //need to return the integer in the last position of the DP array matrix
}
