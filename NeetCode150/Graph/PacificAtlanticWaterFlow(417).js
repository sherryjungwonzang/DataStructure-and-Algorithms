...
var pacificAtlantic = (heights) => {
  ...
  //BFS
  function bfs(queue) {
    //to state whether the value is valid or not
    const isValid = (x, y) => x >= 0 && y >= 0 && x < r && y < c; //valid status
    //specify the directions that we can take - up/down/right/left
    ...
    while(queue.length) {
      ...
      const [x, y] = queue.shift();  //removing the first element
      //update true in the visited array based on the queues' index
      visited[x][y] = true;

      //starting 4 directions
      for (let dir in directions) {
        //extract next
        let nextX = x + dir[0]; //current x + directions at zero
        let nextY = y + dir[1];
      
        //even if it is invalid or being visited - continue
  ...
  for (let x = 0; x < r; x++) {
    for (let y = 0; y < c; y++) {
      if (pacific[x][y] && atlantic[x][y]) { //if there is a duplicate matrix - matching both pacific and atlantic
