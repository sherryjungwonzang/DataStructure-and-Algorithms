...
//return the index ot target if it is in nums, or -1 if it is not in nums
...
var searchRotatedSortedArr = (nums, target) => {
  ...
  while(left <= right) {
    let mid = left + Math.floor((right - left) / 2); //to avoid the stack overflow

    if (nums[mid] === target) {
      return mid; //return index of mid
    }

    //to check which side is sorted
    if (nums[right] > nums[mid]) { //meaning the right side is sorted
      if(target > nums[mid] && target <= nums[right]) {
        left = mid + 1;
      } else {
        right = mid - 1;
      }
    } else { //meaning the left side is sorted
