...
//DFS recursive call - to check all possibl solutions
...
var numOfIslands = (grid) => {
  ...
  for (let i = 0; i < grid.length; i++) { //row
    for (let j = 0; j < grid[i].length; j++) { //column
      if (grid[i][j] === "1") {
        //recursive function - to check all other possible positions
        count = count + dfs(grid, i, j); //i is row, j is column
      }
    }
  }
