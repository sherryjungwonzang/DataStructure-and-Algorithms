...
var decodeWays = (s) => {
  //base cases
  if (s[0] === '0') return 0; //there are no possible solutions
  ...
  for (let i = 2; i <= s.length; i++) {
    ...
    let single = +s[i - 1]; //need to pass it into an integer -> so use + in front of it
    let double = +(s[i - 2] + s[i - 1]);

    //to check it single and double are within range
